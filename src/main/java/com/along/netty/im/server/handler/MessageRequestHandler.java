...
public class MessageRequestHandler extends SimpleChannelInboundHandler<MessageReqPacket> {
    ...
    @Override
    protected void channelRead0(ChannelHandlerContext ctx, MessageReqPacket messageRequestPacket) {
        // 1.拿到消息发送方的会话信息
        Session session = SessionUtil.getSession(ctx.channel());

        // 2.通过消息发送方的会话信息构造要发送的消息
        MessageRespPackage messageResponsePacket = new MessageRespPackage();
